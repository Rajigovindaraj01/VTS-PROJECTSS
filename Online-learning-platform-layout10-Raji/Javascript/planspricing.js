...
document.addEventListener("DOMContentLoaded", () => {
  const buttons = document.querySelectorAll(".sect2-row4");
  const subscribedPlan = localStorage.getItem("subscribedPlan");

  buttons.forEach((button, index) => {
    const planName = button.closest(".section-column").querySelector(".x1").innerText;
    if (subscribedPlan === planName) {
      button.innerText = "Subscribed ✅ Cancel";
      button.style.backgroundColor = "#ccc";
    }
    button.addEventListener("click", () => {
      const currentPlan = button.closest(".section-column").querySelector(".x1").innerText;

      if (localStorage.getItem("subscribedPlan") === currentPlan) {
        localStorage.removeItem("subscribedPlan");
        button.innerText = "Start Subscription";
        button.style.backgroundColor = "";
        alert(`Subscription to ${currentPlan} cancelled.`);
      } else {
        localStorage.setItem("subscribedPlan", currentPlan);
        buttons.forEach(btn => {
          btn.innerText = "Start Subscription";
          btn.style.backgroundColor = "";
        });
        button.innerText = "Subscribed ✅ Cancel";
        button.style.backgroundColor = "#ccc";
        alert(`Subscribed to ${currentPlan} plan.`);
      }
    });
  });
});

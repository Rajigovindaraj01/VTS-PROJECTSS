...
// -----checkouttt---------------------------------------------------------------------------------------------------------------------------------
    const product = JSON.parse(localStorage.getItem("final-learning-details"));

    if (product && product.originalPrice && product.title) {
        const originalPrice = product.originalPrice;
        const title = product.title;
        const discount = (83 / 100) * originalPrice;
        const finalPrice = originalPrice - discount;

        document.getElementById("billOriginalPrice").textContent = `Rs. ${originalPrice}`;
        document.getElementById("billDiscountAmount").textContent = `-Rs. ${discount.toFixed(2)}`;
        document.getElementById("billFinalPrice").textContent = `Rs. ${finalPrice.toFixed(2)}`;
        document.getElementById("billPayAmount").textContent = `${finalPrice.toFixed(2)}`;
        document.getElementById("payNowBtn").addEventListener("click", () => {
            ...
            localStorage.setItem("learning-checkout", JSON.stringify(checkoutData));
            window.location.href = "../Html/confirmed.html";
        });

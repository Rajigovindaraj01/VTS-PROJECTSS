...
const data = JSON.parse(localStorage.getItem("learning-checkout"));
  
  if (data) {
    document.getElementById("title").textContent = data.title || "N/A";
    document.getElementById("originalPrices").textContent = `Rs.${data.originalPrice}`;
    document.getElementById("discount").textContent = `-Rs.${parseFloat(data.discount).toLocaleString()}`;
    document.getElementById("total").textContent = `Rs.${parseFloat(data.total).toLocaleString()}`;
  } else {
    console.log("No data found in localStorage for 'learning-checkout'");
  }
...
document.addEventListener("DOMContentLoaded", () => {
    const checkoutData = JSON.parse(localStorage.getItem("cart-checkout"));
    if (!checkoutData) return;

    const billingContainer = document.querySelector(".ss");
    const addr = checkoutData.deliveryAddress;

    billingContainer.querySelector(".ss1:nth-of-type(1) div:last-child").innerText =
        addr.firstName + " " + addr.lastName;

    billingContainer.querySelector(".ss1:nth-of-type(2) div:last-child").innerText =
        `${addr.address} ${addr.apartment}, ${addr.city}, ${addr.state} - ${addr.pincode}, ${addr.country}`;

    billingContainer.querySelector(".ss1:nth-of-type(3) div:last-child").innerText =
        addr.phone;

    billingContainer.querySelector(".ss1:nth-of-type(4) div:last-child").innerText =
        addr.email || "-";
